      ...
      <div className="relative z-[5]">
        ...
        <main className="max-w-6xl mx-auto px-6 py-4">
          ...
          <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {/* Habits Summary */}
            <DirectionCard title="Habits" icon={CheckSquare} delay={0.25}>
              <div className="space-y-3">
                ...
                <div className="progress-bar">
                  <motion.div
                    className="progress-fill progress-fill-victory"
                    initial={{ width: 0 }}
                    animate={{
                  ...
                  {activeHabits.map(habit => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-sm ${isComplete ? 'text-bone' : 'text-fog'}`}>
                          {habit.name}
                        </span>
                      </button>
                    )
                ...
                {habitStreak > 0 && (
                  <>
                    ...
                    <StreakDisplay count={habitStreak} label="day Steady Course" />
                  </>
                )}
              </div>
            </DirectionCard>
            ...
            {/* Insights Card */}
            <DirectionCard title="Insights" icon={TrendingUp} delay={0.3}>
            ...
            <DirectionCard title="Messages in a Bottle" icon={Mail} delay={0.35}>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-fog text-sm">
                ...
                {arrivedMessages.length > 0 ? (
                  <div className="space-y-3 max-h-64 overflow-y-auto">
                    {unreadMessages.length > 0 && (
                      <p className="text-xs text-fjord font-medium">
                        {unreadMessages.length} new message{unreadMessages.length > 1 ? 's' : ''} arrived
                      </p>
                    )}
                    {arrivedMessages.slice(0, 3).map(message => (
                      <MessageCard
                        key={message.id}
                        message={message}
                    ...
                    {arrivedMessages.length > 3 && (
                      <p className="text-xs text-stone text-center">
                        +{arrivedMessages.length - 3} more messages
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="glass-recessed p-6 text-center">
                    <Mail className="w-8 h-8 text-stone mx-auto mb-2 opacity-50" />
                    <p className="text-stone text-sm">
                      {pendingMessages.length > 0
                        ? 'Your messages are sailing toward you...'
            ...
            {/* Bearings */}
            <DirectionCard title="Bearings" icon={BookOpen} delay={0.4}>
              <div className="space-y-4">
                <p className="text-fog text-sm">
                  Weekly and monthly reflections to keep you on course.
                </p>

                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => openBearingModal('weekly')}
                    className="btn-secondary flex flex-col items-center justify-center gap-1 py-3"
                  >
                    <Calendar className="w-4 h-4" />
                    <span className="text-xs">Weekly</span>
                    {currentWeekBearing && (
                      <span className="text-xs text-victory-green">✓</span>
                    )}
                  </button>
                  <button
                    onClick={() => openBearingModal('monthly')}
                    className="btn-secondary flex flex-col items-center justify-center gap-1 py-3"
                  >
                  ...
                  <div className="space-y-3 max-h-64 overflow-y-auto">
                    {recentBearings.map(bearing => (
                      <BearingDisplayCard
                        key={bearing.id}
                        bearing={bearing}
                        onEdit={() => openBearingModal(bearing.type)}
                      />
                    ))}
                  </div>
                ) : (
                  <div className="glass-recessed p-6 text-center">
                    <BookOpen className="w-8 h-8 text-stone mx-auto mb-2 opacity-50" />
                    <p className="text-stone text-sm">
                      No reflections yet. Start with a weekly check-in!
                    </p>
                  </div>
                )}
              </div>
            ...
            <DirectionCard title="True North" icon={Star} delay={0.45}>
                <div className="space-y-3">
                  <button
                    onClick={() => {
                      setEditingGoal(undefined)
                    ...
                    <div className="space-y-2 max-h-[250px] overflow-y-auto">
                      {lifeGoals.map(goal => (
                        <LifeGoalCard
                          ...
                          onUpdateProgress={(p) => handleUpdateGoalProgress(goal.id, p)}
                          onDelete={() => handleDeleteLifeGoal(goal.id)}
                        />
                      ))}
                    </div>
                  ) : (
                    <div className="glass-recessed p-4 text-center">
                      <Star className="w-6 h-6 text-stone mx-auto mb-1 opacity-50" />
            ...
            <DirectionCard title="Waypoints" icon={Flag} delay={0.5}>
              <div className="space-y-4">
                <p className="text-fog text-sm">
                  Celebrate milestones on your journey.
                </p>
                ...
                {recentWaypoints.length > 0 ? (
                  <div className="space-y-3 max-h-[300px] overflow-y-auto">
                    {recentWaypoints.map(wp => (
                      <WaypointCard
                        key={wp.id}
                        waypoint={wp}
                        linkedGoal={lifeGoals.find(g => g.id === wp.goal_id)}
                  ...
                  <div className="glass-recessed p-6 text-center">
                    <Flag className="w-8 h-8 text-stone mx-auto mb-2 opacity-50" />
                    <p className="text-stone text-sm">
                      No milestones logged yet. Celebrate your achievements!
                    </p>
                  </div>
                )}

                {waypoints.length > 0 && (
                  <div className="text-center pt-2 border-t border-iron-slate/50">
                    <span className="text-xs text-stone">
                      {waypoints.length} milestone{waypoints.length !== 1 ? 's' : ''} logged
                    </span>
                  </div>
                )}
              </div>
